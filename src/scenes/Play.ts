...
addToCreate(){
	...
	window.fillRect(this.cameras.main.width/2 - 100, this.cameras.main.height/2 - 75, 200, 30);
	...
	const graveText = {
		...
		"21, 5": "Here rests Sparky, who followed every squirrel to the great beyond",
		"21, 12": "In loving memory of Charlie, who ran free at the Rainbow Bridge",
		"22, 8": "Here lies Daisy, who gave more kisses than any dog could count",
	...
	this.physics.add.overlap(this.player, graveLayer!, () => {
		...
		if (tile?.properties.Interactable === true) {
			...
			tis.grave_interact(tile, epitaph, graveText);
		} else {
			window.setVisible(false);
			epitaph.setVisible(false);
		}
...
grave_interact(grave: Phaser.Tilemaps.Tile, epitext: Phaser.GameObjects.Text, library: object){	// displays black box with epitaph
	const key = grave?.x.toString() + ", " + grave?.y.toString();
	epitext.text = library[key];
}
